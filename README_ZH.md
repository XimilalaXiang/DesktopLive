...
### 🎯 核心功能
- **实时转录** - 捕获系统音频，即时转换为文字
- **🆕 实时屏幕字幕** - 悬浮字幕窗口，在任何屏幕上显示实时转录
- **多服务商支持** - 支持 Soniox、火山引擎等多个 ASR 服务提供商
- **多语言支持** - 支持中文、英文及 60+ 种语言

### 📝 字幕功能（v1.0.3 新增）
- **悬浮字幕** - 始终置顶的透明窗口，实时显示转录内容
- **自定义样式** - 调整字体大小、字体类型、文字颜色、背景颜色
- **文字阴影** - 可选文字阴影，提升可读性
- **拖拽移动** - 鼠标悬停显示锁定按钮，解锁后可拖拽调整位置
- **智能换行** - 根据窗口宽度自动换行
- **行数控制** - 可配置最大显示行数（1-5 行）

### 📤 导出与管理
- **历史记录** - 按日期/时间分组，支持自定义标题和标签
- **导出为 TXT** - 一键导出转录内容为文本文件
- **🆕 导出为 SRT** - 导出带时间戳的 SRT 字幕文件
- **数据备份** - 支持导入/导出数据，轻松迁移
...
- **界面语言** - 支持中英文界面切换
- **自动更新** - 支持自动检查和下载 GitHub Releases 更新
